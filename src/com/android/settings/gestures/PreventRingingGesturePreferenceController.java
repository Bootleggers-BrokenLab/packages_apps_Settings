...
import static android.provider.Settings.Secure.BOOTLEGGERS_VOLUME_HUSH_MUTE;
import static android.provider.Settings.Secure.BOOTLEGGERS_VOLUME_HUSH_NORMAL;
import static android.provider.Settings.Secure.BOOTLEGGERS_VOLUME_HUSH_VIBRATE;
...
public class PreventRingingGesturePreferenceController extends AbstractPreferenceController
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (settingsValue == null) settingsValue = BOOTLEGGERS_VOLUME_HUSH_OFF;
        ArrayList<String> currentValue = new ArrayList<String>();
        currentValue.addAll(Arrays.asList(settingsValue.split(",", 0)));

        if (isAdd) {
            if (currentValue.get(0).equals(BOOTLEGGERS_VOLUME_HUSH_OFF))
                currentValue.clear();
            if (!currentValue.contains(preventRingingSetting))
                currentValue.add(preventRingingSetting);
        } else {
            if (currentValue.size() == 1 ||
                    preventRingingSetting.equals(BOOTLEGGERS_VOLUME_HUSH_OFF)) {
                currentValue.clear();
                currentValue.add(BOOTLEGGERS_VOLUME_HUSH_OFF);
    ...
    public void updateState(Preference preference) {
        ...
        final boolean isVibrate = enabled && preventRingingSetting.contains(BOOTLEGGERS_VOLUME_HUSH_VIBRATE);
        final boolean isMute = enabled && preventRingingSetting.contains(BOOTLEGGERS_VOLUME_HUSH_MUTE);
        final boolean isNormal = enabled && preventRingingSetting.contains(BOOTLEGGERS_VOLUME_HUSH_NORMAL);
    ...
    private String keyToSetting(String key) {
        switch (key) {
            case KEY_MUTE:
                return BOOTLEGGERS_VOLUME_HUSH_MUTE;
            case KEY_VIBRATE:
                return BOOTLEGGERS_VOLUME_HUSH_VIBRATE;
            case KEY_NORMAL:
                return BOOTLEGGERS_VOLUME_HUSH_NORMAL;
            default:
                return BOOTLEGGERS_VOLUME_HUSH_OFF;
        }
    }
